...
export const retrievalWithCapability = (q: string) => {
  const cases = TRAINING_CASES.filter(x => x.intent.toLowerCase().includes(q.toLowerCase()) || x.designProblem.toLowerCase().includes(q.toLowerCase()));
  const cap = require('./capability-map').mapDecision(q);
  return { cases, cap, realOnly: !!cap || cases.length>0 };
};
