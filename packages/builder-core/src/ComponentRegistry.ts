...
export const STANDARD_COMPONENT_DESCRIPTORS: ReadonlyArray<ComponentDescriptor> = [
  ...
  },
  {
    ...
    allowChildren: false,
    schema: [
      stringProp({ key: 'src', label: 'Adres URL wideo', required: true, group: 'content', defaultValue: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4' }),
      stringProp({ key: 'poster', label: 'Obrazek Miniaturka (Poster)', required: false, group: 'media', defaultValue: '' }),
      booleanProp({ key: 'controls', label: 'Pokaż sterowanie', required: false, group: 'behavior', defaultValue: true }),
      booleanProp({ key: 'autoPlay', label: 'Autoodtwarzanie', required: false, group: 'behavior', defaultValue: false }),
      booleanProp({ key: 'loop', label: 'Pętla', required: false, group: 'behavior', defaultValue: true }),
      booleanProp({ key: 'muted', label: 'Wyciszone', required: false, group: 'behavior', defaultValue: true }),
      stringProp({ key: 'width', label: 'Szerokość (np. 100%, 400px)', required: false, group: 'layout', defaultValue: '100%' }),
      stringProp({ key: 'height', label: 'Wysokość (np. auto, 300px)', required: false, group: 'layout', defaultValue: '300px' }),
    ...
    defaultProps: { src: 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4', poster: '', controls: true, autoPlay: false, loop: true, muted: true, width: '100%', height: '300px', objectFit: 'cover', borderRadius: '12px' },
    tags: ['video', 'film', 'media', 'wideo'],
  },
  {
    type: 'video-player',
