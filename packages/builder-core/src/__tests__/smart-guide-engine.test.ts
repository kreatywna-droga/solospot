...
describe('DistanceCalculator', () => {
  ...
  it('should show vertical distance between elements', () => {
    const input = makeInput(
      { id: 'drag', x: 0, y: 200, height: 100 },
      [{ id: 'target', x: 0, y: 0, height: 100 }] // drag top = 200, target bottom = 100 → distance = 100
    );
    const engine = new SmartGuideEngine();
    const result = engine.computeAll(input);
    expect(result.distanceCount).toBeGreaterThanOrEqual(1);
...
describe('SpacingCalculator', () => {
  it('should detect equal horizontal spacing', () => {
    // Three elements: left (0,0,w=100), drag (150,0,w=100), right (300,0,w=100)
    // Left gap = 150-100 = 50, Right gap = 300-250 = 50 → equal
    const input = makeInput(
  ...
  it('should detect equal vertical spacing', () => {
    const input = makeInput(
      { id: 'drag', x: 0, y: 150, height: 80 },
      [
        { id: 'top', x: 0, y: 0, height: 80 },
        { id: 'bottom', x: 0, y: 310, height: 80 },
      ]
    );
    // Top gap = 150-80 = 70, Bottom gap = 310-230 = 80 → not equal (difference=10 > threshold=8)
    // Change to make equal: drag.y = 160, top.height = 80 → top bottom = 80, gap = 80
    // bottom.y = 320, gap = 320-240 = 80
    const input2 = makeInput(
    ...
    const engine = new SmartGuideEngine();
    const result2 = engine.computeAll(input2);
    expect(result2.spacingCount).toBeGreaterThanOrEqual(1);
  });
