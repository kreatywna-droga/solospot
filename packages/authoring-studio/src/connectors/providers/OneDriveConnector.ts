/**
 * OneDriveConnector.ts — Sprint S9 Real Connector Implementation (ETAP 4)
 *
 * OneDrive Cloud Connector implementation.
 *
...
export class OneDriveConnector implements CloudStorageConnectorContract {
  ...
  upload(request: CloudStorageUploadRequest): CloudStorageUploadResult {
    return createCloudStorageUploadResult(this.connectorId, true, request.item.itemId, `https://onedrive.live.com/files/${request.item.itemId}`);
  }
