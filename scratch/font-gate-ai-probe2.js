/**
 * FONT GATE v1.0 â€” AI-path probe: force an underspecified/design font prompt
...
const PROMPT = process.env.PROMPT || 'dobierz mi eleganckÄ… szeryfowÄ… czcionkÄ™ do tego nagĹ‚Ăłwka';
...
(async () => {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const st = await page.evaluate(() => {
      const p = document.querySelector('[data-testid="mini-inspector-ai"]');
      const t = (window.__SOLOSPOT_LATENCY_TRACES__ || []);
      return { status: p && p.getAttribute('data-ai-status'), message: p && (p.innerText||'').split('\n').slice(4,9).join(' | ').slice(0,300), traceCount: t.length, last: t[t.length-1] && {path:t[t.length-1].path, status:t[t.length-1].executionStatus, notes:t[t.length-1].notes} };
    });
    console.log('POLL', i, JSON.stringify(st));
    if (st.status && st.status !== 'EXECUTING' && st.status !== 'PENDING') break;
  }
  await saveDoc(page);
