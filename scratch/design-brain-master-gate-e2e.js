/**
 * HACP Design Brain Master Gate v1.0 — Dental E2E
 *
 * Extends knowledge-gate E2E with DesignBrain runtime assertions:
 * - [DesignBrain] provenance log in console
 * - designBrain status on plan (via generation complete signal)
 * - Knowledge baseline still present ([Knowledge])
 * - Mutation (no fake SUCCESS)
 * - No batch_execute
 * - Dental content + booking CTA
 * - Console errors
 */
...
const OUT_DIR = path.join(__dirname, 'design-brain-master-gate-proof');
const EXACT_PROMPT =
...
(async () => {
  ...
  const consoleErrors = [];
  const knowledgeLogs = [];
  const designBrainLogs = [];
  const allLogs = [];
  page.on('pageerror', (err) => consoleErrors.push(err.message));
  page.on('console', (msg) => {
    const text = msg.text();
    if (msg.type() === 'error') consoleErrors.push(text);
    allLogs.push(text.slice(0, 600));
    if (text.includes('[Knowledge]')) knowledgeLogs.push(text.slice(0, 500));
    if (text.includes('[DesignBrain]')) designBrainLogs.push(text.slice(0, 500));
  });
  ...
  if (!ta) {
    ...
    log('A1', 'Exact design-brain gate prompt sent', 'implantologii + estetycznej + umówienia wizyty', 'sent', 'PASS', EXACT_PROMPT.slice(0, 80));
  }
  ...
  log(
    ...
    'schema=1.0.0 with industry/blueprint',
    knLog ? knLog.slice(0, 180) : 'missing',
    /schema=1\.0\.0/.test(knLog) && /blueprint=BP-/.test(knLog) ? 'PASS' : knLog ? 'WARN' : 'FAIL',
    knLog.slice(0, 240),
  );

  // DesignBrain runtime log — may appear as server-side console or client if attached
  const dbLog =
    designBrainLogs.find((l) => l.includes('[DesignBrain]')) ||
    allLogs.find((l) => l.includes('[DesignBrain]')) ||
    '';
  log(
    'A2b',
    '[DesignBrain] runtime provenance log',
    'status= + version= present',
    dbLog ? dbLog.slice(0, 180) : 'missing-or-server-only',
    dbLog
      ? /status=/.test(dbLog) && /v=1\.0\.0|version/.test(dbLog)
        ? 'PASS'
        : 'WARN'
      : 'WARN',
    dbLog.slice(0, 240),
  ...
  log('A8', 'Booking CTA present', 'Umów wizytę-like CTA', ctaBook ? 'yes' : 'missing', ctaBook ? 'PASS' : 'FAIL');
  ...
  // DesignBrain was soft-attached: either runtime log or mutation with knowledge log
  const designBrainSignal = designBrainLogs.length > 0 || dbLog.length > 0 || knowledgeLogs.length > 0;
  log(
    'A14',
    'DesignBrain soft-attach did not block generation',
    'generation succeeded with knowledge/db provenance',
    `mutated=${mutated}, knowledgeLogs=${knowledgeLogs.length}, designBrainLogs=${designBrainLogs.length}`,
    mutated && designBrainSignal ? 'PASS' : 'FAIL',
  );
  ...
  const evidence = {
    ...
    designBrainLogs: designBrainLogs.slice(0, 10),
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      { baseUrl: BASE_URL, prompt: EXACT_PROMPT, results: RESULTS, evidence, trace: TRACE.slice(0, 80) },
      null,
      2,
    ),
  );

  console.log(`\n=== DESIGN BRAIN MASTER GATE E2E: ${pass} PASS / ${fail} FAIL / ${warn} WARN ===`);
  await browser.close();
