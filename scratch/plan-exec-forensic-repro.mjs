...
async function main() {
  ...
  try {
    ...
    const hasMutation = finalToolCalls.some((tc) =>
      /^(insert_|update_|set_|remove_|move_|delete_|batch_|configure_)/.test(tc.name)
    );
    const textOnly = firstToolCalls.length === 0 && Boolean(first?.content);

    L('12_POINT_RECORD', {
      '1_user_request': PROMPT,
      '2_selected_model': orchResult.modelUsed || first?.model || 'unknown',
      '3_intent': classified.category,
      '4_capability_surface': tools,
      '5_tools_passed_to_model': turns.find((t) => t.phase === 'REQUEST')?.toolsSent || [],
      '6_raw_model_response': (first?.content || '').slice(0, 800),
      '7_model_returned_tool_call': firstToolCalls.length > 0,
      '8_model_returned_text_plan': textOnly,
      '9_continuation_started': responses.length > 1,
      '10_next_model_turn_executed': responses.length > 1,
      '11_tool_call_executed': finalToolCalls.length > 0,
      '12_final_status': orchResult.status,
      finalToolCalls: finalToolCalls.map((tc) => tc.name),
      finalHasMutation: hasMutation,
      planWithoutExecution: textOnly && finalToolCalls.length === 0,
      modelTurnCount: responses.length,
    });
  } finally {
